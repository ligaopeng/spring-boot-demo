...
public class WebLogAspect {
    ...
    @Pointcut("execution(public * com.lgp.controller.*.*.*(..)) && !execution(public * com.lgp.controller.*.MqController.*(..)) && !execution(public * com.lgp.controller.*.UploadController.*(..))")
//定义一个切入点
    public void webLog() {
    }
...
//在切入点return内容之后切入内容，可以用来处理加工返回值，@Around在切入点前后切入内容，并自己控制何时执行切入点自身的内容，@AfterThrowing用来处理切入内容部分跑出异常之后的处理逻辑
